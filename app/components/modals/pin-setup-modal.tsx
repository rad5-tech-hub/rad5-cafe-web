...
export const PinSetupModal: React.FC<PinSetupModalProps> = ({ isOpen, onDismiss, onDone }) => {
  ...
  const handleSubmit = async () => {
    ...
    setError(null);
    try {
      const res = await api.auth.setupPin(code);
      if (res.success) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs">
      ...
      >
        <div className="text-center flex flex-col gap-1">
          <h3 className="text-xl font-bold text-text-main leading-tight">Set Up Transaction PIN</h3>
          <p className="text-text-secondary text-sm leading-normal">
            Create a 4-digit PIN to authorize payments
          </p>
        </div>
